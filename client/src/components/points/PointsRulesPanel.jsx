    ...
    <div className="pd-rules-accordion">
      ...
      >
        ...
        <svg
          ...
          strokeWidth="2.5"
          strokeLinecap="round"
          style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 200ms ease', flexShrink: 0, opacity: 0.55 }}
        >
...
const PointsRulesPanel = () => (
  <div className="points-rules">
    ...
    <div className="points-rules__header">
      <div className="points-rules__header-left">
        <span className="points-rules__badge">Official Record</span>
        <h3>Point Rules &amp; Transparency</h3>
        <p>{pointSystemConfig.semester} · Maintained by VPAA</p>
      </div>
      <div className="points-rules__header-meta">
        <span className="points-rules__doc-label">Chapter Policy Document</span>
      </div>
    </div>

    {/* Two-column grid */}
    <div className="points-rules__grid">

      {/* Left column: policy sections */}
      <div className="points-rules__col">
          ...
          <ul>
            {[...STREAK_TIERS].sort((a, b) => a.threshold - b.threshold).map((tier) => (
              <li key={tier.key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span>
                  {tier.threshold}+ meetings → <strong>{tier.multiplier}x</strong> multiplier on chapter points
                </span>
              </li>
            ))}
          </ul>
      ...
      <div className="points-rules__col">
        <p className="points-rules__col-label">Event Categories</p>
